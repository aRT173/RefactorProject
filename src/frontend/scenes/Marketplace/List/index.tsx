...
import { AdditionalInfo, CardInfo } from "./styles";
...
export const List = (props: ListProps) => {
  ...
  useEffect(() => {
    ...
    if (visibleCards.length >= items.length) setHasMore(false);
  ...
  const fetchMore = () => {
    setVisibleCards((prev) => [
      ...prev,
      ...items.slice(prev.length - 1, prev.length + VISIBLE_CARDS_COUNT),
    ]);
      ...
      {visibleCards.map((value, index) => (
        ...
        >
          ...
          <CardInfo>
            ...
            <AdditionalInfo>
              <div>
                <strong>Rooms:</strong> {value.roomsNumber}
              </div>
              <div>
                <strong>Floor:</strong> {value.floor}
              </div>
              <div>
